...
def view_syllabus_page():
    st.title("Syllabus Overview")
    
    st.subheader("Introductory Video")
    video_path = r"C:\Users\SAP Library\Downloads\vid\vid1-1.mp4"
    if os.path.exists(video_path):
        ...
        st.warning(f"Video not found at the specified path: {video_path}")
            ...
            for course in courses:
                if search_term.lower() in course['name'].lower():
                    with st.expander(course['name'], expanded=False):
                        modules = db.get_syllabus_modules(course['id'])
                        if not modules:
                        ...
                        for module in modules:
                            if search_term.lower() in module['title'].lower():
                                ...
                                lessons = db.get_syllabus_lessons(module['id'])
                                if lessons:
...
def view_trainee_performance_section():
    ...
    if 'score' in df.columns and 'total' in df.columns:
        df['accuracy'] = df.apply(
            lambda row: (row['score'] / row['total'] * 100) if row['total'] > 0 else 0, 
            axis=1
        ).round(2)
        
        quizzes = db.get_all_quizzes()
        quiz_cat_map = {q['name']: q['category'] for q in quizzes}
        df['category'] = df['quiz_name'].map(quiz_cat_map)
        ...
        col1, col2 = st.columns(2)
        col1.metric("Average Accuracy", f"{avg_accuracy:.2f}%")
        ...
        st.error("The user progress data is missing 'score' or 'total' columns. Please check the `get_all_user_progress` function in `database.py` to ensure it returns these columns.")
        st.subheader("Debug Info")
        st.write("Columns found in DataFrame:", df.columns.tolist())
        st.dataframe(df)
